...
public:
	...
	{
		return BVTNavData_GetSource( m_ptr );
	}

	/** Stores a user-defined string which identifies the NavData source, 
	 * e.g. "MRU", "GPS", "Compass". <br>.
	 */
	RetVal SetSource(std::string source_string)
	{
		return BVTNavData_SetSource( m_ptr, source_string.c_str() );
	}

	/** Returns the stored string, in null-terminated form. If no
	 * string was stored, returns a null string.
	 */
	std::string GetUTMLongitudeZone()
	{
		return BVTNavData_GetUTMLongitudeZone( m_ptr );
	}

	/** Stores a user-defined string which identifies the UTM Zone used as the
	 * basis for Northing and Easting values.<br>.
	 */
	RetVal SetUTMLongitudeZone(std::string zone_string)
	{
		return BVTNavData_SetUTMLongitudeZone( m_ptr, zone_string.c_str() );
	}

	/** Stores a heave value, which is expected to be supplied by an MRU.
	 * \param heave_in_meters 
	 */
	RetVal SetHeave(float heave_in_meters)
	{
		return BVTNavData_SetHeave( m_ptr, heave_in_meters );
	}

	/** Gets the stored heave. If no value was stored for this ping,
	 * returns BVT_NAV_NO_DATA.
	 */
	RetVal GetHeave(float* heave_in_meters)
	{
		return BVTNavData_GetHeave( m_ptr, heave_in_meters );
	}
